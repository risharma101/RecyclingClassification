...
if __name__ == '__main__':
    ...
    for folderName in categoryFileList:
        ...
        for imgName in imgFileList:
            ...
            labels = np.append(labels,labels.shape[0])
    

            edgeshist = edginess.extractEdges(img)
            edgeshist = np.append(edgeshist, np.zeros(data.shape[1] - edgeshist.shape[0]))
            data = np.vstack((data,np.expand_dims(edgeshist, axis=0)))
            print('##################')
            print(data.shape)
            print(labels.shape)
            ...
            label = None
            #SAVE OUTPUT
            if not os.path.exists('featureInfoEdge'):
                os.makedirs('featureInfoEdge')
            
            # np.save(os.path.join('featureInfoHSV',"{0}_{1}".format(folderName,imgName.split(".")[0])),data)
            np.save(os.path.join('featureInfoEdge',imgName.split(".")[0]),data)

            arr = np.load(os.path.join('featureInfoEdge',imgName.split(".")[0] + '.npy'))
